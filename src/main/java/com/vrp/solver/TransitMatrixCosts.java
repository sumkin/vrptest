...
public class TransitMatrixCosts extends AbstractForwardVehicleRoutingTransportCosts {

    private static final double LARGE_COST = 1e9;

    private final Map<String, TransitLane> laneMap;

    public TransitMatrixCosts(Map<String, TransitLane> laneMap) {
    ...
    public double getTransportTime(Location from, Location to, double departureTime, Driver driver, Vehicle vehicle) {
        ...
        String asset = vehicle != null ? vehicle.getType().getTypeId() : null;
        TransitLane lane = lookup(from.getId(), to.getId(), asset);
        return lane != null ? lane.travelTimeHours * 3600.0 : LARGE_COST;
    }
    ...
    public double getDistance(Location from, Location to, double departureTime, Vehicle vehicle) {
        ...
        String asset = vehicle != null ? vehicle.getType().getTypeId() : null;
        TransitLane lane = lookup(from.getId(), to.getId(), asset);
        return lane != null ? lane.distance : LARGE_COST;
    }
